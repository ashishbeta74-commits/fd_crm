...
const STAGE_PATTERNS = [
  ...
  ['hung_up', /hung\s*up|hang\s*up|hangup|cut\s*(the\s*)?call|call\s*cut|dropped\s*(the\s*)?call|disconnected\s*(the\s*)?call/],
  ['not_interested', /not\s*interested|no\s*need|don'?t\s*need|dont\s*need|do\s*not\s*need|not\s*needed|not\s*required|no\s*requirement|declined|refused|do\s*not\s*call|\bdnc\b|not\s*relevant|no\s*thanks|not\s*looking/],
  ...
  ['started', /\bstart|\bcalled\b|attempt|contacted|in\s*progress|follow|trying|tried|next\s*week/],
